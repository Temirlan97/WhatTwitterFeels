...
def generateTimeline(startDate, endDate, interval):
	...
	while(itDate < endDate):
		...
		itDate += interval
	return timeline
#postfixes = ["12h-7d", "1d-7d"]
interval = datetime.timedelta(days = 1)
frameWidth = datetime.timedelta(days = 7)

startDateTr = datetime.datetime(2018, 03, 31)
endDateTr = datetime.datetime(2018,04,20)
timelineTr = generateTimeline(startDateTr, endDateTr, interval)

startDateTest = endDateTr
endDateTest = datetime.datetime.now() - datetime.timedelta(days=1)
timelineTest = generateTimeline(startDateTest, endDateTest, interval)
...
try:
	...
	for t in (timelineTr + timelineTest):
		while True:
			if(priceItem[0] < t):
				closestLowerPriceItem = priceItem
				priceItem = itPrice.next()
...
def inputOutputMatrices(startDate, endDate, inputMatrixArray, outputMatrixArray):
	...
	while(frameUpper < endOn):
		...
		futurePrices = getPriceChange(frameUpper, frameUpper + frameWidth)
		try:
...
inputMatrixArray = []
outputMatrixArray = []
inputOutputMatrices(startDateTr, endDateTr, inputMatrixArray, outputMatrixArray)
Phi = np.array(inputMatrixArray)
Z = np.array(outputMatrixArray)
del inputMatrixArray
del outputMatrixArray
stopwatchEnd = time.time()
infoLog("Computing Phi and Z took: " + str(stopwatchEnd - stopwatchStart))


infoLog("Computing PhiTest and ZTest...")
stopwatchStart = time.time()
inputMatrixArray = []
outputMatrixArray = []
inputOutputMatrices(startDateTest, endDateTest, inputMatrixArray, outputMatrixArray)
PhiTest = np.array(inputMatrixArray)
ZTest = np.array(outputMatrixArray)
stopwatchEnd = time.time()
infoLog("Computing PhiTest and ZTest took: " + str(stopwatchEnd - stopwatchStart))


infoLog("Saving Phi and Z...")
stopwatchStart = time.time()
prefix = "data/Computed/Into_two/"
postfix = "_1d-7d"
np.save(prefix + "trainingInput" + postfix, Phi)
np.save(prefix + "trainingOutput" + postfix, Z)
np.save(prefix + "testingInput" + postfix, PhiTest)
np.save(prefix + "testingOutput" + postfix, ZTest)
stopwatchEnd = time.time()
