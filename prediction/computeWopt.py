...
def analyzeError(name, Phi, Z, PhiTest, ZTest):
	...
	print("Side accuracy: " + str(correctSide*100.0/sideTotal) + "%")

postfixesStandard = ["1m-1h", "5m-1h", "1h-1d", "12h-3d", "12h-7d", "1d-7d"]
postfixes = ["1m-1h"]
prefix = "data/Computed/Long/FramewiseFrame/"
print(prefix)
for intervalAndFrame in postfixesStandard:
	...
	for i in range(0, len(Phi)):
		PhiPrices.append(Phi[i][41:])
		PhiTweets.append(Phi[i][:41])
	PhiTestPrices = []
	PhiTestTweets = []
	for i in range(0, len(PhiTest)):
		PhiTestPrices.append(PhiTest[i][41:])
		PhiTestTweets.append(PhiTest[i][:41])
